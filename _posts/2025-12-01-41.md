...
```
이 while 루프가 도는 동안, 다른 쓰레드가 같은 세션에 `sendMessage()`를 호출하면 프레임이 섞입니다
...
같은 세션 ID에 대해 동일한 락 객체를 공유하므로, 한 쓰레드가 `sendMessage()`를 호출 중이면 다른 쓰레드는 `synchronized` 블록에서 대기합니다. 서로 다른 세션끼리는 락이 독립적이므로 병렬 전송에 영향을 주지 않습니다.
